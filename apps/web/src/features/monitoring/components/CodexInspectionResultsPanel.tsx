...
export function CodexInspectionResultsPanel({
  ...
  const renderOperationForItem = (item: CodexInspectionResultItem) =>
    renderOperation ? (
      ...
      >
        {formatAction ? formatAction(item.action) : formatActionLabel(item.action, t)}
      </Button>
    ) : item.action === 'reauth' ? (
              ...
              <div className={styles.resultsToolbarActions}>
                {onDeleteReauthPlanned ? (
                  <Button
                    ...
                    disabled={
                      !result || isInspectionInFlight || executing || reauthActionCount === 0
                    }
                  >
                    <IconTrash2 size={14} />
                ...
                <Button
                  ...
                  loading={executing}
                  disabled={
                    !result || isInspectionInFlight || executing || pendingActionCount === 0
                  }
                >
                  {headerButtonText}
                </Button>
          ...
          <div className={styles.resultCardList}>
            {filteredResults.length > 0 ? (
              filteredResults.map((item) => {
                ...
                const operation = showActionControls ? renderOperationForItem(item) : null;
                return (
                  <article
                    key={item.key}
